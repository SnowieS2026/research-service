...
async function scrapeArticle(url: string): Promise<ScrapedArticle> {
  ...
  try {
    ...
    const { stderr } = await execAsync(
      `python "${SCRAPE_SCRIPT}" < "${urlFile}" > "${outFile}"`,
    ...
    const data = JSON.parse(out.slice("SCRAPER_RESULT:".length)) as { title: string; desc: string; pubdate: string; content: string };
    ...
    try { fs.unlinkSync(urlFile); } catch { /* nop */ }
    try { fs.unlinkSync(outFile); } catch { /* nop */ }
  }
}

// Fallback: Node native fetch for metadata only
async function fetchMetadata(url: string, domain: string): Promise<ScrapedArticle> {
  try {
    ...
    if (!resp.ok) return { url, title: "Untitled", domain, content: "", success: false, error: `HTTP ${resp.status}` };
...
export function deduplicateArticles(articles: ScrapedArticle[]): ScrapedArticle[] {
