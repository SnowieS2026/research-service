...
exports.formatBeeHiiv = exports.formatTikTok = exports.formatXThread = exports.formatX = exports.formatSubstack = void 0;
exports.parseNewsletter = parseNewsletter;
...
const platform_formatters_js_1 = require("./platform-formatters.js");
Object.defineProperty(exports, "formatSubstack", { enumerable: true, get: function () { return platform_formatters_js_1.formatSubstack; } });
Object.defineProperty(exports, "formatX", { enumerable: true, get: function () { return platform_formatters_js_1.formatX; } });
Object.defineProperty(exports, "formatXThread", { enumerable: true, get: function () { return platform_formatters_js_1.formatXThread; } });
Object.defineProperty(exports, "formatTikTok", { enumerable: true, get: function () { return platform_formatters_js_1.formatTikTok; } });
Object.defineProperty(exports, "formatBeeHiiv", { enumerable: true, get: function () { return platform_formatters_js_1.formatBeeHiiv; } });
// ── Markdown Parser ─────────────────────────────────────────────────────────
function parseNewsletter(md, issueNumber = 1) {
    ...
    function extract(rawMd, startRx, endRx) {
    ...
    function parseStories(rawMd, startRx, endRx) {
    ...
    function parseBullets(rawMd, startRx, endRx) {
        const section = extract(rawMd, startRx, endRx);
        return section
            .split("\n")
    ...
    function parseSignals(rawMd) {
        ...
        const contentLines = [];
        for (const line of rawLines) {
            if (/^## SIGNALS FROM THE EDGE$/mi.test(line)) {
                inSignals = true;
                continue;
            }
    ...
    function extractHook(rawMd) {
        const rawLines = rawMd.split("\n");
        let inHook = false;
        const result = [];
        for (const line of rawLines) {
            if (/^#{1,3}\s+[^#]/.test(line) && !line.startsWith("# PRISMAL")) {
                inHook = true;
                continue;
            }
            if (inHook && line.startsWith("#"))
                break;
            if (inHook && line.trim())
                result.push(line.trim());
            if (inHook && result.length >= 2)
                break;
        }
...
function mdToHtml(text) {
    ...
    for (const block of blocks) {
        ...
        // Horizontal rule
        if (trimmed === "---") {
            htmlBlocks.push("<hr style='border:none;border-top:1px solid #333;margin:16px 0;'>");
            continue;
        }
...
function buildDashboard(data, outputs, beehiivHtml) {
    ...
    function renderTextarea(platform, label, charCount, limit, overLimit) {
    ...
    const clipboardData = JSON.stringify(Object.fromEntries(outputs.map(o => [o.platform, o.copyText])));
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
// ── CLI ──────────────────────────────────────────────────────────────────────
function generateDashboard(mdPath, outputDir, issueNumber = 1) {
    const md = fs.readFileSync(mdPath, "utf8");
    const data = parseNewsletter(md, issueNumber);
    const beehiivFormatted = (0, platform_formatters_js_1.formatBeeHiiv)(data);
    const beehiivHtml = beehiivFormatted.content;
    const outputs = [
        (0, platform_formatters_js_1.formatSubstack)(data),
        (0, platform_formatters_js_1.formatX)(data),
        (0, platform_formatters_js_1.formatXThread)(data),
        (0, platform_formatters_js_1.formatTikTok)(data),
        beehiivFormatted,
    ];
