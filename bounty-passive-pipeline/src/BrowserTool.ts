...
export class BrowserTool {
   ...
   */
  async processFixture(fixturePath: string): Promise<DiffNotification> {
    ...
    const program: NormalisedProgram = await parser.parse(page, fixturePath);
    ...
    const prevHash = prevHashes.find((h: string) => h !== hash);
    ...
    if (prevHash && prevHash !== hash) {
      const prevProg = (await this.snapshotMgr.load(identifier, prevHash)) as NormalisedProgram;
      const diff = diffPrograms(prevProg, program, prevHash, hash, identifier);
      diffResult = {
        oldHash: diff.oldHash,
        newHash: diff.newHash,
        addedFields: diff.addedFields.map((f: { field: string }) => f.field),
        removedFields: diff.removedFields.map((f: { field: string }) => f.field),
        changedFields: diff.changedFields.map((f: { field: string }) => f.field)
      };
