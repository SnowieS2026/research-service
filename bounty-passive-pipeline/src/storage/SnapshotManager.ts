...
export class SnapshotManager {
  ...
  async store(data: unknown, identifier: string): Promise<string> {
  ...
  async load(identifier: string, hash: string): Promise<unknown> {
  ...
  async list(identifier: string): Promise<string[]> {
    let files: string[] = [];
    try {
