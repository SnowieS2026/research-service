...
import { type DiscoveryResult, discoverPrograms } from './ProgramDiscovery.js';
...
interface CliArgs {
  mode: 'watch' | 'once' | 'single-program' | 'single-platform' | 'discover-only' | 'list-platforms';
  programUrl?: string;
  platform?: string;
  scan?: boolean;
}
...
async function runPipeline(cfg: ReturnType<typeof loadConfig>, cli: CliArgs): Promise<void> {
  ...
  LOG.log(
    `Pipeline summary: ${totalPrograms} programs scanned, ` +
...
async function discoverPlatformPrograms(platform: string): Promise<string[]> {
  const browser = new MetadataBrowser();
  await browser.init();
  try {
    const result: DiscoveryResult = await discoverPrograms(platform);
...
async function fetchAndDiff(
  ...
  if (prevHash && prevHash !== newHash) {
    ...
    diffResult.removedFields = diff.removedFields.map((f: { field: string }) => f.field);
    diffResult.changedFields = diff.changedFields.map((f: { field: string }) => f.field);
  }
...
function extractProgramSlug(url: string): string {
  try {
    const u = new URL(url);
...
async function runDiscovery(cfg: ReturnType<typeof loadConfig>): Promise<void> {
...
async function deliverScanReport(
  ...
  for (const f of result.findings) {
    if (f.severity in bySev) bySev[f.severity as keyof typeof bySev]++;
  }
  ...
  if (techs.length > 0) {
    ...
    for (const [tech, count] of techs) lines.push(`- **${tech}** (${count})`);
    lines.push('');
  }
  ...
  for (let i = 0; i < Math.min(sorted.length, 20); i++) {
    ...
    const param = (f as unknown as Record<string, unknown>).param ?? '';
  ...
  if (cfg.ENABLE_TELEGRAM && !cfg.DRY_RUN) {
    const summary = [
      ...
      ...sorted.slice(0, 5).map(f =>
        `${f.severity === 'CRITICAL' ? '🔴' : f.severity === 'HIGH' ? '🟠' : '🟡'} [${f.severity}] ${f.type} – ${f.url.substring(0, 50)}`
      ),
      '',
      `📄 Full report: \`${mdPath}\``
    ].join('\n');
