...
export class VehicleCollector {
  ...
  private async collectGovUkMot(
    ...
    try {
      ...
      if (advisories.length > 0) rawData['govuk_advisories'] = advisories;
  ...
  private async collectMotors(
    ...
    try {
      ...
      for (const m of bodyText.matchAll(priceRe)) {
        ...
        if (p > 500 && p < 100000) prices.push(p);
      }
      if (prices.length > 0) {
        prices.sort((a, b) => a - b);
        const min = prices[0];
  ...
  private async collectMotorCheck(
    plate: string,
    _findings: OsintFinding[],
    _errors: string[]
  ): Promise<CollectorResult> {
