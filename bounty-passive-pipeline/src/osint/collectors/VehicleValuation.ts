...
export type Urgency = 'immediate' | 'soon' | 'when_due' | 'advisory';

export interface AdvisoryCost {
  item: string;
  severity: Severity;
  urgency: Urgency;
  estimatedCostMin: number; // GBP — independent garage, 2025-2026
...
export interface VehicleValuation {
  ...
  valueWithAdvisoriesMin: number;
  valueWithAdvisoriesMax: number;
  expectedMonthsRemaining: number;
  motFailRisk: 'high' | 'medium' | 'low';
  totalAdvisoryCostMin: number;
  totalAdvisoryCostMax: number;
...
interface AdvisoryEntry {
  keywords: string[];      // lowercase substrings to match in MOT text
  item: string;
  severity: Severity;
  urgency: Urgency;
  costMin: number;
...
const ADVISORY_DATABASE: AdvisoryEntry[] = [
  // ── Tyres ────────────────────────────────────────────────────────────────────
  {
 ...
 */
function matchAdvisories(cleanedText: string): AdvisoryCost[] {
  const lower = cleanedText.toLowerCase();
  const matched: AdvisoryCost[] = [];
 ...
 */
export function estimateAdvisories(
...
export function estimateValue(
  ...
  const depreciatedBase = (base: number) => {
  ...
  const condMult: Record<string, number> = { poor: 0.50, fair: 0.70, good: 1.0 };
...
export function estimateLifespan(
  ...
  motFailures: number,
  fuelType: string,
  seriousAdvisories: number = 0  // critical or high severity in CURRENT advisories
): { months: number; assessment: string } {
  const age = new Date().getFullYear() - year;
 ...
 */
export function generateVehicleValuation(
  make: string,
  model: string,
  year: number,
  mileage: number | null,
  fuelType: string,
  advisories: AdvisoryCost[],
  motFailures: number,
  motTotal: number
): VehicleValuation {
  ...
  const condition: 'good' | 'fair' | 'poor' =
    hasCritical ? 'poor' : hasHigh ? 'fair' : advisories.length <= 2 ? 'good' : 'fair';
  ...
  let motFailRisk: 'high' | 'medium' | 'low' = 'low';
