...
export class IpCollector {
  async collect(query: OsintQuery): Promise<CollectorResult> {
    ...
    try {
      ...
      const geo = tryParseJson<Record<string, unknown>>(geoText, {});
    ...
    try {
      ...
      const bgp = tryParseJson<Record<string, unknown>>(bgpText, {});

      rawData['bgp'] = bgp;

      if ((bgp.status as string) === 'ok') {
        const data = bgp.data as Record<string, unknown> | undefined;
        if (data) {
          const prefixes = data.prefixes as Array<Record<string, unknown>> | undefined;
          if (prefixes && prefixes.length > 0) {
            findings.push({
              source: 'BGPView',
              field: 'bgp_prefix',
              value: prefixes.map((p: Record<string, unknown>) => String(p.prefix ?? '')).join(', '),
              confidence: 90
            });
            const routeObj = prefixes[0] as Record<string, unknown>;
            const asnData = routeObj.related_asn_infos as Array<Record<string, unknown>> | undefined;
    ...
    try {
      ...
      const rdap = tryParseJson<Record<string, unknown>>(rdapText, {});
    ...
    try {
      ...
      const ptrJson = tryParseJson<Record<string, unknown>>(ptrText, {});

      const answers = ptrJson.Answer as Array<Record<string, unknown>> | undefined;
  ...
  private async grabBanners(
    ...
    const grab = (port: number): Promise<string | null> =>
      new Promise((resolve) => {
