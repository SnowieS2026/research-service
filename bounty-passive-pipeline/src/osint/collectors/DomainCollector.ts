  ...
  async collect(query: OsintQuery): Promise<CollectorResult> {
    ...
    for (const rtype of dnsTypes) {
      try {
        ...
        const dnsJson = tryParseJson<Record<string, unknown>>(text, {});
        const dnsData = (dnsJson.Answer ?? dnsJson.Answer ?? []) as Array<Record<string, unknown>>;

        if (dnsData.length > 0) {
          const values = dnsData
            .map((a: Record<string, unknown>) => String(a.data ?? ''))
            .filter(Boolean);
    ...
    const aRecords = (rawData.dns_A as string[]) ?? [];
    ...
    try {
      ...
      const whois = tryParseJson<Record<string, unknown>>(text, {});

      if (whois.WhoisRecord) {
        const wr = whois.WhoisRecord as Record<string, unknown>;
        rawData.whois = wr;
        findings.push({
          source: 'WHOIS',
          field: 'registrar',
    ...
    for (const tld of commonTlds) {
      ...
      try {
        ...
        const dns = tryParseJson<Record<string, unknown>>(text, {});
        const answers = dns.Answer as Array<Record<string, unknown>> | undefined;
