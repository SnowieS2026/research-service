    ...
    if (cfg.OSINT_NUMVALIDATE_KEY) {
      try {
        ...
        if (data.valid === true || data.valid === 'true') {
          ...
          if (data.country_name) findings.push({ source: 'NumValidate', field: 'country', value: String(data.country_name), confidence: 90 });
          if (data.location) findings.push({ source: 'NumValidate', field: 'location', value: String(data.location), confidence: 75 });
          if (data.carrier) findings.push({ source: 'NumValidate', field: 'carrier', value: String(data.carrier), confidence: 80 });
          if (data.line_type) findings.push({ source: 'NumValidate', field: 'line_type', value: String(data.line_type), confidence: 85 });
        } else {
