...
interface PlatformCheck {
  name: string;
  urlTemplate: (u: string) => string;
  existsRe?: RegExp;  // if matches in response HTML → account exists
  existsReNeg?: RegExp; // if matches → account does NOT exist
}
...
export class UsernameCollector {
  async collect(query: OsintQuery): Promise<CollectorResult> {
    ...
    let existsCount = 0;
    const results: Array<{ platform: string; url: string; exists: boolean; status?: number }> = [];
