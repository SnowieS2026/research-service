  ...
  async collect(query: OsintQuery): Promise<CollectorResult> {
    ...
    if (cfg.OSINT_HIBP_API_KEY) {
      try {
        ...
        const breaches = tryParseJson<Array<{ Name: string; Title: string; BreachDate: string }>>(text, []);
    ...
    if (cfg.OSINT_HUNTER_API_KEY && domain) {
      try {
        ...
        const emails = (data.data as Record<string, unknown>)?.emails as Array<Record<string, unknown>> | undefined;
        if (Array.isArray(emails) && emails.length > 0) {
          ...
          for (const e of emails.slice(0, 3)) {
            findings.push({
              ...
              confidence: 75,
              url: e.sources ? (e.sources as Array<Record<string, unknown>>)[0]?.domain as string ?? undefined : undefined
            });
          }
        }
      } catch (err) {
