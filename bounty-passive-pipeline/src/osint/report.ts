...
export function generateOsintReport(result: OsintResult): string {
  ...
  if (result.query.type === 'vehicle') {
    ...
    const rawVal = (result.rawData['CarCheck'] as Record<string, unknown>)?.['vehicle_valuation'] as any;
    const rawAdvisories = (result.rawData['CarCheck'] as Record<string, unknown>)?.['advisory_costs'] as any[] | undefined;
    ...
    const colour = (result.rawData['CarCheck'] as Record<string, unknown>)?.['Colour']
    ...
    const motPassed = (result.rawData['CarCheck'] as Record<string, unknown>)?.['mot_passed']
      ?? result.findings.find(f => f.field === 'mot_passed')?.value;
    const motFailed = (result.rawData['CarCheck'] as Record<string, unknown>)?.['mot_failed']
    ...
    if (costFindings.length > 0 || (rawAdvisories && rawAdvisories.length > 0)) {
      ...
      if (rawAdvisories && rawAdvisories.length > 0) {
        ...
        lines.push(`| --- | --- | --- | --- | --- |`);
        rawAdvisories.forEach((ac: any, i: number) => {
