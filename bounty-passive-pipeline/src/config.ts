...
export interface PipelineConfig {
  ...
  DRY_RUN: boolean;
  MIN_SEVERITY_TO_NOTIFY: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
...
function isValidSeverity(s: string): s is PipelineConfig['MIN_SEVERITY_TO_NOTIFY'] {
  ...
  for (const [key, value] of Object.entries(override)) {
    if (value !== undefined && value !== null) {
      if (Array.isArray(value)) {
        ...
        result[key] = deepMerge(result[key] as PipelineConfig, value as Partial<PipelineConfig>);
      } else {
        result[key] = value;
      }
    }
...
export function loadConfig(configPath?: string): PipelineConfig {
  ...
  try {
    if (fs.existsSync(configFilePath)) {
      ...
      fileConfig = JSON.parse(raw) as Partial<PipelineConfig>;
    }
  } catch (err) {
  ...
  const envConfig: Partial<PipelineConfig> = {};

  if (process.env.DRY_RUN !== undefined) envConfig.DRY_RUN = process.env.DRY_RUN !== 'false';
  if (process.env.RATE_LIMIT_DELAY_MS !== undefined) {
    const n = parseInt(process.env.RATE_LIMIT_DELAY_MS, 10);
    if (!isNaN(n)) envConfig.RATE_LIMIT_DELAY_MS = n;
  }
  if (process.env.WATCH_INTERVAL_MS !== undefined) {
    const n = parseInt(process.env.WATCH_INTERVAL_MS, 10);
    if (!isNaN(n)) envConfig.WATCH_INTERVAL_MS = n;
  }
  if (process.env.MIN_SEVERITY_TO_NOTIFY !== undefined) {
    const s = process.env.MIN_SEVERITY_TO_NOTIFY;
    if (isValidSeverity(s)) envConfig.MIN_SEVERITY_TO_NOTIFY = s;
  }
  if (process.env.ENABLE_TELEGRAM !== undefined) envConfig.ENABLE_TELEGRAM = process.env.ENABLE_TELEGRAM !== 'false';
  if (process.env.ENABLE_DB !== undefined) envConfig.ENABLE_DB = process.env.ENABLE_DB !== 'false';
  if (process.env.SNAPSHOT_STORE !== undefined) envConfig.SNAPSHOT_STORE = process.env.SNAPSHOT_STORE;
  if (process.env.REPORTS_DIR !== undefined) envConfig.REPORTS_DIR = process.env.REPORTS_DIR;
...
export function meetsSeverityThreshold(
  severity: PipelineConfig['MIN_SEVERITY_TO_NOTIFY'],
  minSeverity: PipelineConfig['MIN_SEVERITY_TO_NOTIFY']
): boolean {
