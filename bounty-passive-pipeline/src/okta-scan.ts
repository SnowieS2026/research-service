...
function loadProbeSubdomains(): string[] {
  const probeFiles = [
    ...
    if (fs.existsSync(fp)) {
      try {
        ...
        if (data.live_hosts && Array.isArray(data.live_hosts)) {
          ...
          return data.live_hosts.map((h: string) => (h.startsWith('http') ? h : `https://${h}`));
        }
      } catch { /* ignore */ }
    }
  }
...
function buildTargets(args: string[]): string[] {
...
// ── Run scan ─────────────────────────────────────────────────────────────────
async function runScan(targets: string[]): Promise<void> {
  if (targets.length === 0) {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(dir, `okta-scan-${hash}.json`),
    ...
    if (result.findings.length > 0) {
      ...
      for (const f of sorted.slice(0, 30)) {
        ...
        if (f.references?.length) mdLines.push(`- **References**: ${f.references.join(', ')}`);
        mdLines.push('');
      }
    } else {
