...
import { type ApiFinding, buildFindingId } from './ScanResult.js';
 ...
 */
async function probeGraphQL(endpoint: DiscoveredEndpoint): Promise<GraphQLResult> {
  ...
  try {
    ...
    const json = await response.json() as { data?: { __schema?: { queryType?: { name?: string }; mutationType?: { name?: string } } } };
 ...
 */
async function probeRESTApi(baseUrl: string): Promise<ApiFinding[]> {
