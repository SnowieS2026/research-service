...
function isInterestingTitle(title: string | null): boolean {
...
function isInterestingStatus(statusCode: number): boolean {
  ...
  if (statusCode >= 500) return true; // 5xx often expose debug/error pages
  if (statusCode >= 400 && statusCode < 500) return true; // 401, 403, etc. can be interesting
  if (statusCode >= 300 && statusCode < 400) return true; // redirects
  return false;
}
 ...
 */
export async function scanHttpx(
  targets: string[],
  _stack: unknown,
