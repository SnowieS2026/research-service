 ...
 *   -t <dir>   — template directory (can be repeated for multiple dirs)
...
/** Template sub-dirs to run from the nuclei-templates directory. */
const TEMPLATE_DIRS = [
  'vulnerabilities/',
  'exposed-panels/',
  ...
  for (const line of output.split('\n').filter(Boolean)) {
    try {
      ...
      if (textMatch) {
        findings.push(
          nucleiToFinding(
            textMatch[3], textMatch[2], textMatch[1],
            new Date().toISOString(), \`Nuclei match: \${textMatch[2]}\`, line
          )
        );
      }
    }
...
`;
fs.writeFileSync('C:/Users/bryan/.openclaw/workspace/bounty-passive-pipeline/src/scanner/NucleiScanner.ts', content);
