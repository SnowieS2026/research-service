...
export class IntigritiParser extends BaseParser {
    ...
    async parse(page, url) {
        ...
        const rewardRange = await this.safeExtract(page, '[data-testid="reward-range"], .reward-range, .bounty-range, [data-testid="bounty-range"], .payout-range');
        ...
        const lastSeenAt = await this.safeExtract(page, '[data-testid="last-updated"], .last-updated, time, tr:has-text("Updated") td:last-child');
