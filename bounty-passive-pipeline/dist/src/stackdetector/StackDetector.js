...
const SIGNATURES = [
    // ── CDN / WAF ────────────────────────────────────────────────────────────
    {
        ...
        category: 'cdn',
        confidence: 95,
        detect: ({ headers }) => headerContains(headers, 'server', 'cloudflare') ||
            !!headers['cf-ray'] ||
    ...
    },
    {
        ...
        category: 'cdn',
        confidence: 90,
        detect: ({ headers }) => headerContains(headers, 'server', 'cloudfront') ||
            !!headers['x-amz-cf-id']
    },
    {
        name: 'Fastly',
        category: 'cdn',
        confidence: 90,
        detect: ({ headers }) => headerContains(headers, 'server', 'fastly') ||
    ...
    },
    {
        ...
        category: 'cdn',
        confidence: 85,
        detect: ({ headers }) => headerContains(headers, 'server', 'akamai') ||
    ...
    },
    {
        ...
        category: 'cdn',
        confidence: 90,
        detect: ({ headers }) => !!headers['x-sucuri-id'] ||
    ...
    },
    {
        ...
        category: 'cdn',
        confidence: 80,
        detect: ({ headers }) => !!headers['x-cdn']?.toLowerCase().includes('stackpath')
    },
    // ── Servers ──────────────────────────────────────────────────────────────
    {
        name: 'nginx',
        category: 'server',
        confidence: 90,
        detect: ({ headers }) => headerContains(headers, 'server', 'nginx')
    },
    {
        name: 'Apache',
        category: 'server',
        confidence: 85,
        detect: ({ headers }) => headerContains(headers, 'server', 'apache') &&
    ...
    },
    {
        ...
        category: 'server',
        confidence: 90,
        detect: ({ headers }) => headerContains(headers, 'server', 'microsoft-iis') ||
    ...
    },
    {
        ...
        category: 'server',
        confidence: 90,
        detect: ({ headers }) => headerContains(headers, 'server', 'caddy')
    },
    {
        name: 'Kestrel',
        category: 'server',
        confidence: 80,
        detect: ({ headers }) => headerContains(headers, 'server', 'kestrel')
    },
    // ── Languages / Backends ─────────────────────────────────────────────────
    {
        name: 'PHP',
        category: 'backend',
        confidence: 85,
        detect: ({ headers, url }) => headerContains(headers, 'x-powered-by', 'php') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 75,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'express') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 60,
        detect: ({ headers, html }) => headerContains(headers, 'x-powered-by', 'python') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 60,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'phusion passenger') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 70,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'jrun') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 80,
        detect: ({ headers, url }) => headerContains(headers, 'server', 'asp.net') ||
    ...
    },
    {
        ...
        category: 'backend',
        confidence: 60,
        detect: ({ headers }) => headerContains(headers, 'server', 'net/http') ||
    ...
    // ── Frameworks ──────────────────────────────────────────────────────────
    {
        ...
        category: 'frontend',
        confidence: 95,
        detect: ({ html, url }) => /\_next\/static|__next\.js|next\.js|next-router/.test(html) ||
            /\/api\//.test(url)
    },
    {
        name: 'Nuxt.js',
        category: 'frontend',
        confidence: 95,
        detect: ({ html }) => /__nuxt__|nuxt\.js|_nuxt/.test(html)
    },
    {
        name: 'React',
        category: 'frontend',
        confidence: 85,
        detect: ({ html, jsSnippets }) => /react[@."]*[0-9]|ReactRouter|react-dom|_react_/i.test(html) ||
    ...
    },
    {
        ...
        category: 'frontend',
        confidence: 85,
        detect: ({ html, jsSnippets }) => /vue[@."]*[0-9]|vue-router|vuex|_vue_/i.test(html) ||
    ...
    },
    {
        ...
        category: 'frontend',
        confidence: 85,
        detect: ({ html, jsSnippets }) => /angular[@."]*[0-9]|ng-app|ng-controller|angular\.module/i.test(html) ||
    ...
    },
    {
        ...
        category: 'frontend',
        confidence: 80,
        detect: ({ html }) => /svelte[@."]*[0-9]|svelte\.js|_svelte_/i.test(html)
    },
    {
        name: 'jQuery',
        category: 'frontend',
        confidence: 90,
        detect: ({ html, jsSnippets }) => /jquery[@."]*[0-9]|jquery-[0-9]|jQuery/i.test(html) ||
    ...
    },
    {
        ...
        category: 'frontend',
        confidence: 80,
        detect: ({ html }) => /__debug__|django\.forms|csrfmiddlewaretoken/.test(html)
    },
    {
        name: 'Flask',
        category: 'frontend',
        confidence: 75,
        detect: ({ html }) => /flask|jinja/i.test(html)
    },
    {
        name: 'Laravel',
        category: 'frontend',
        confidence: 90,
        detect: ({ html, url }) => /laravel|sanctum|__cf_challenge__/i.test(html) ||
    ...
    },
    {
        ...
        category: 'frontend',
        confidence: 80,
        detect: ({ html }) => /spring-boot|springframework|thymeleaf/i.test(html)
    },
    // ── CMS ─────────────────────────────────────────────────────────────────
    {
        name: 'WordPress',
        category: 'cms',
        confidence: 95,
        detect: ({ html, url }) => /wp-content|wp-includes|wordpress|wp-json\/oembed/i.test(html) ||
    ...
    },
    {
        ...
        category: 'cms',
        confidence: 95,
        detect: ({ html }) => /drupal|drupalorg|drupal\.settings|node\/\d+|sites\/default/i.test(html)
    },
    {
        name: 'Joomla',
        category: 'cms',
        confidence: 95,
        detect: ({ html }) => /joomla|com_content|option=com_/i.test(html)
    },
    {
        name: 'Magento',
        category: 'cms',
        confidence: 95,
        detect: ({ html }) => /magento|skin\/frontend|checkout\/cart|loggedInCustomer/i.test(html)
    },
    {
        name: 'Shopify',
        category: 'cms',
        confidence: 95,
        detect: ({ headers, html }) => headerContains(headers, 'server', 'cloudflare') && /myshopify\.com|cdn\.shopify\.com/i.test(html) ||
    ...
    // ── API ──────────────────────────────────────────────────────────────────
    {
        ...
        category: 'api',
        confidence: 95,
        detect: ({ html, url }) => /graphql|__schema|__type|IntrospectionQuery/i.test(html) ||
    ...
    },
    {
        ...
        category: 'api',
        confidence: 60,
        detect: ({ html, url }) => /swagger|openapi|api\/docs|\/api\/v|\/rest\//i.test(html) ||
    ...
    },
    {
        ...
        category: 'api',
        confidence: 80,
        detect: ({ html }) => /trpc|trpcInvoke|@trpc\/server/i.test(html)
    },
    {
        name: 'Swagger UI',
        category: 'api',
        confidence: 90,
        detect: ({ html }) => /swagger-ui|swagger-ui-react|swagger-ui\/dist|swagger-resources/i.test(html)
    },
    // ── Databases ────────────────────────────────────────────────────────────
    {
        name: 'MySQL',
        category: 'database',
        confidence: 50,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'mysql') ||
    ...
    },
    {
        ...
        category: 'database',
        confidence: 50,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'postgres') ||
    ...
    },
    {
        ...
        category: 'database',
        confidence: 50,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'mongodb') ||
    ...
    },
    {
        ...
        category: 'database',
        confidence: 50,
        detect: ({ headers }) => headerContains(headers, 'x-powered-by', 'redis') ||
    ...
    // ── Auth ─────────────────────────────────────────────────────────────────
    {
        ...
        category: 'auth',
        confidence: 70,
        detect: ({ html, headers }) => /\beyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/.test(html) ||
    ...
    },
    {
        ...
        category: 'auth',
        confidence: 75,
        detect: ({ html }) => /oauth|authorize\?client_id=|signin\/oauth/i.test(html)
    },
    {
        name: 'Session Cookies',
        category: 'auth',
        confidence: 60,
        detect: ({ html }) => /sessionid|session_id|PHPSESSID|JSESSIONID|ASP\.NET_SessionId/i.test(html)
    }
];
// ── Known favicon hashes ──────────────────────────────────────────────────────
const FAVICON_HASHES = {
...
export class StackDetector {
     ...
     */
    async detect(url, maxRedirects = 5) {
        ...
        try {
            ...
            for (const [k, v] of response.headers.entries()) {
                ...
                if (k.toLowerCase() === 'server' && v)
                    server = v;
            }
            // Detect CDN/WAF
            if (interestingHeaders['cf-ray'])
                cdn = 'Cloudflare';
            else if (interestingHeaders['x-amz-cf-id'])
                cdn = 'AWS CloudFront';
            else if (interestingHeaders['x-served-by']?.includes('fastly'))
                cdn = 'Fastly';
            else if (interestingHeaders['x-akamai-transformed'])
                cdn = 'Akamai';
            else if (interestingHeaders['x-sucuri-id'])
                cdn = 'Sucuri';
            html = await response.text();
            // Check for path-based signatures
            for (const sig of PATH_SIGNATURES) {
                if (sig.pattern.test(finalUrl) || sig.pattern.test(html)) {
                    if (!frontend.includes(sig.name) && sig.category === 'frontend')
                        frontend.push(sig.name);
                    if (sig.category === 'cms')
                        cms = sig.name;
            ...
            try {
                if (sig.detect(ctx)) {
                    ...
                    if (!seen.has(key)) {
                        ...
                        // Categorise
                        if (sig.category === 'frontend' && !frontend.includes(sig.name))
                            frontend.push(sig.name);
                        if (sig.category === 'backend' && !backend.includes(sig.name))
                            backend.push(sig.name);
                        if (sig.category === 'cms')
                            cms = sig.name;
                        if (sig.category === 'server')
                            server = sig.name;
                        if (sig.category === 'language')
                            language = sig.name;
                        if (sig.category === 'framework')
                            framework = sig.name;
                    }
                }
            }
    ...
    buildRecommendations(techs, ctx) {
        const recs = [];
