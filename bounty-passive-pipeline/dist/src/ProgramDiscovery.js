 ...
 */
export async function discoverPrograms(platform) {
...
function getListingUrl(platform) {
    switch (platform) {
        case 'bugcrowd':
...
async function scrapeProgramLinks(page, platform) {
    const seen = new Set();
    ...
    if (seen.size === 0) {
        try {
            const allLinks = await page.locator('a[href]').evaluateAll((els) => els.map((el) => el.href));
...
function getProgramLinkSelectors(platform) {
    switch (platform) {
        case 'bugcrowd':
            return [
                'a[href^="/programs/"]',
...
function normalizeProgramUrl(href, platform) {
    if (!href)
        return null;
    try {
