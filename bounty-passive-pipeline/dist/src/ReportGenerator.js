...
function todayDir(reportsDir) {
    const today = new Date().toISOString().split('T')[0];
    const dir = path.join(reportsDir, today);
    fs.mkdirSync(dir, { recursive: true });
    return dir;
}
function buildChangeEntries(diff) {
    const entries = [];
    const diffAny = diff;
...
function buildRecommendations(classification, diff) {
    const recs = [];
    if (classification.newAssets.length > 0) {
        recs.push(`**Scope expansion detected** – ${classification.newAssets.length} new asset(s) added. ` +
    ...
    if (classification.rewardChanged === 'increased') {
        recs.push('**Bounty increased** – Reward ranges have gone up. ' +
    ...
    if (classification.rewardChanged === 'decreased') {
        recs.push('**Bounty decreased** – Reward ranges have been reduced. ' +
    ...
    if (classification.changedFields.includes('prohibited_techniques')) {
        recs.push('**Technique restrictions eased** – Previously prohibited techniques may now be allowed. ' +
    ...
    if (classification.changedFields.includes('allowed_techniques')) {
        recs.push('**New allowed techniques** – The program has explicitly listed new acceptable techniques. ' +
            'Incorporate these into your testing workflow.');
    }
    if (classification.severity === 'HIGH' || classification.severity === 'CRITICAL') {
        recs.push('**High-impact change detected** – This change warrants immediate attention. ' +
            'Prioritise this program in your current testing cycle.');
    }
    if (recs.length === 0) {
        recs.push('Review the raw diff below for full details on what changed.', 'Subscribe to notifications for this program to stay updated.');
    }
    return recs;
}
export function generateReport(diff, classification) {
...
export function writeReport(data, reportsDir, diffHash) {
...
function buildSummary(data) {
    const parts = [];
    if (data.newAssets.length > 0) {
        parts.push(`${data.newAssets.length} new ${data.newAssets.length === 1 ? 'asset' : 'assets'} ` +
            `added to the scope of ${data.programName} on ${data.platform}.`);
    }
    if (data.removedAssets.length > 0) {
        parts.push(`${data.removedAssets.length} ${data.removedAssets.length === 1 ? 'asset' : 'assets'} ` +
            `removed from scope.`);
    }
    ...
    if (parts.length === 0) {
        parts.push(`Minor metadata change detected for ${data.programName}. ` +
