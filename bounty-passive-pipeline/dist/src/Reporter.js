...
export class Reporter {
    ...
    async process(notification) {
        ...
        LOG.log(`Classification for ${notification.program_name}: ${classification.severity} ` +
            `(${classification.reasons.join(', ')})`);
        if (!meetsSeverityThreshold(classification.severity, this.config.MIN_SEVERITY_TO_NOTIFY)) {
            LOG.log(`Severity ${classification.severity} below threshold ${this.config.MIN_SEVERITY_TO_NOTIFY} – skipping`);
            return null;
        }
    ...
    async saveToDb(reportData, paths, notification) {
        if (!this.db)
            return;
        try {
            const reportId = this.db.insertReport(this.runId, reportData.programName, reportData.platform, reportData.severity, reportData.cvss, paths.mdPath, notification.diff.newHash);
            for (const change of reportData.changes) {
                this.db.insertFinding(reportId, change.field, change.oldValue, change.newValue, '');
            }
    ...
    async sendToTelegram(reportData, paths) {
        try {
    ...
    buildTelegramMessage(data, paths) {
        const lines = [];
