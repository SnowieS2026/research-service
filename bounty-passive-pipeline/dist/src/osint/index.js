...
export async function runOsint(query) {
    ...
    const results = await Promise.allSettled(collectors.map(collector => runCollectorWithTimeout(collector, query, timeoutMs)));
    const allFindings = [];
...
async function runCollectorWithTimeout(collector, query, timeoutMs) {
