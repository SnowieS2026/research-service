...
function extractDdValue(html, label) {
...
export class VehicleCollector {
    ...
    async collectUK(plate, findings, errors, rawData) {
        try {
            ...
            if (make)
                findings.push({ source: 'GovUK-DVLA', field: 'make', value: make, confidence: 95 });
            if (model)
                findings.push({ source: 'GovUK-DVLA', field: 'model', value: model, confidence: 90 });
            if (colour)
                findings.push({ source: 'GovUK-DVLA', field: 'colour', value: colour, confidence: 95 });
            if (year)
                findings.push({ source: 'GovUK-DVLA', field: 'year', value: year, confidence: 90 });
            if (motStatus)
                findings.push({ source: 'GovUK-DVLA', field: 'mot_status', value: motStatus, confidence: 95 });
            if (motExpires)
                findings.push({ source: 'GovUK-DVLA', field: 'mot_expires', value: motExpires, confidence: 95 });
            if (taxStatus)
                findings.push({ source: 'GovUK-DVLA', field: 'tax_status', value: taxStatus, confidence: 95 });
            if (taxDue)
                findings.push({ source: 'GovUK-DVLA', field: 'tax_due', value: taxDue, confidence: 80 });
            if (fuelType)
                findings.push({ source: 'GovUK-DVLA', field: 'fuel_type', value: fuelType, confidence: 85 });
            if (engineSize)
                findings.push({ source: 'GovUK-DVLA', field: 'engine_size', value: engineSize, confidence: 80 });
