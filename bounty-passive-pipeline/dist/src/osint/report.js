...
export function generateOsintReport(result) {
    ...
    if (result.query.type === 'vehicle') {
        const rawCarCheck = result.rawData['CarCheck'];
        const rawDvla = result.rawData['DVLA'];
        const rawVal = rawCarCheck?.['vehicle_valuation'];
        const rawAdvisories = rawCarCheck?.['advisory_costs'];
        const rawText = rawCarCheck?.['raw'] ?? '';
        // ── Core vehicle fields ────────────────────────────────────────────────────
        const make = rawVal?.['make'] ?? findFinding(result, 'make') ?? 'Unknown';
        const model = rawVal?.['model'] ?? findFinding(result, 'model') ?? 'Unknown';
        const year = rawVal?.['year'] ?? findFinding(result, 'year') ?? '?';
        const colour = rawCarCheck?.['Colour'] ?? findFinding(result, 'colour') ?? rawDvla?.['colour'] ?? 'Unknown';
        const fuelType = rawCarCheck?.['fuel_type'] ?? findFinding(result, 'fuel_type') ?? 'Unknown';
        const engineCc = rawCarCheck?.['engine_capacity'] ?? findFinding(result, 'engine_capacity');
        const gearbox = rawCarCheck?.['gearbox'] ?? findFinding(result, 'gearbox') ?? 'Unknown';
        const bodyType = rawCarCheck?.['body_type'] ?? findFinding(result, 'body_type');
        // ── DVLA fields ───────────────────────────────────────────────────────────
        const dvlaTaxStatus = rawDvla ? extractDvlaTaxStatus(rawDvla) : null;
        const dvlaColour = rawDvla?.['colour'];
        // ── MOT fields ─────────────────────────────────────────────────────────────
        ...
        const motExpiryStr = rawCarCheck?.['mot_expiry'] ?? findFinding(result, 'mot_expiry');
        ...
        const valMin = rawVal?.['currentValueMin'] ?? (valMinVal ? parseInt(valMinVal) : undefined);
        const valMax = rawVal?.['currentValueMax'] ?? (valMaxVal ? parseInt(valMaxVal) : undefined);
        const valAdvMin = rawVal?.['valueWithAdvisoriesMin'];
        const valAdvMax = rawVal?.['valueWithAdvisoriesMax'];
        const expectedMonthsVal = findFinding(result, 'expected_months');
        const expectedMonths = rawVal?.['expectedMonthsRemaining'] ?? (expectedMonthsVal ? parseInt(expectedMonthsVal) : undefined);
        const totalAdvMinVal = findFinding(result, 'advisory_total_min');
        const totalAdvMin = rawVal?.['totalAdvisoryCostMin'] ?? (totalAdvMinVal ? parseInt(totalAdvMinVal) : undefined);
        const totalAdvMaxVal = findFinding(result, 'advisory_total_max');
        const totalAdvMax = rawVal?.['totalAdvisoryCostMax'] ?? (totalAdvMaxVal ? parseInt(totalAdvMaxVal) : undefined);
        const motFailRisk = rawVal?.['motFailRisk'] ?? findFinding(result, 'mot_fail_risk') ?? 'unknown';
        const recommendation = rawVal?.['recommendation'] ?? findFinding(result, 'recommendation');
        // ── Parse MOT history from raw text ───────────────────────────────────────
        // (rawCarCheck and rawText already declared above in vehicle block)
        const collectorTimeline = rawCarCheck?.['mileage_timeline'];
        ...
        if (mileageAnomaly)
            riskFlags.push(`⚠️ Mileage anomaly: ${mileageAnomaly}`);
        // MOT gaps
        const motGaps = detectMotGaps(motRows);
        if (motGaps)
            riskFlags.push(`⚠️ MOT gaps: ${motGaps}`);
        // Repeated failures
        if (motFailed >= 3)
            riskFlags.push(`⚠️ Repeated failures: ${motFailed} fails on record`);
        // Plate transfers
    ...
    function findFinding(result, field) {
    ...
    function extractDvlaTaxStatus(dvla) {
        const status = dvla['tax_status'];
        if (status)
            return status;
        const taxed = dvla['taxed'];
        const sorn = dvla['sorn'];
        if (taxed === false && sorn === true)
            return 'SORN';
        if (taxed === true)
            return 'Taxed';
        if (taxed === false)
    ...
    function extractV5Date(rawText) {
    ...
    function parseMotHistory(rawText) {
        const motRows = [];
        ...
        while ((match = motBlockRe.exec(rawText)) !== null) {
            ...
            if (!seen.has(key)) {
                ...
                if (testCentre !== 'Unknown')
                    testCentres.push(testCentre);
            }
        }
    ...
    function detectMileageAnomaly(timeline) {
    ...
    function calcRiskScore(opts) {
        ...
        if (motExpiryDate) {
            ...
            if (daysToExpiry < 0)
                ...
                score += Math.round((60 - daysToExpiry) / 4); // up to +15 within 2 months
        }
        ...
        for (let i = 1; i < normTimeline.length; i++) {
            if (normTimeline[i] < normTimeline[i - 1]) {
                score += 20;
                break;
            }
        }
    ...
    function buildComparisonTable(allResults) {
        ...
        for (const r of allResults) {
            if (r.query.type !== 'vehicle')
                continue;
            const rc = r.rawData['CarCheck'];
            const rawText = rc?.['raw'] ?? '';
            ...
            const expiryStr = rc?.['mot_expiry'] ?? findFinding(r, 'mot_expiry');
            const expiryDate = expiryStr ? parseMotDate(expiryStr) : null;
            const timeline = rc?.['mileage_timeline'] ?? [];
    ...
    function analyseMileageTrend(timeline) {
    ...
    function extractComparables(rawText) {
        const results = [];
        const patterns = [
    ...
    function getKnownIssuesForModel(make, model) {
    ...
    function estimateOwnerCount(mileageTimeline, motRows, testCentres) {
        if (motRows.length === 0)
            return 'Unable to estimate (no MOT data)';
    ...
    function buildOwnershipIndicators(mileageTimeline, motRows, testCentres) {
        const indicators = [];
    ...
    function inferUsagePattern(locations) {
        if (locations.length === 0)
            return 'Unable to determine (no location data)';
