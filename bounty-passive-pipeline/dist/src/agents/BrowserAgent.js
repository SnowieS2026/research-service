...
export class BrowserAgent extends BaseAgent {
    ...
    async handleMessage(msg) {
        switch (msg.type) {
            ...
            case 'REPAIR:DONE': {
                ...
                if (payload.issueKey.includes('browser') ||
    ...
    // ── Core fetch logic ────────────────────────────────────────────────────────
    async fetchAndDiff(programUrl, platform, isFastTrack, attempt) {
    ...
    async fetchWithRetry(programUrl, platform) {
        if (!this.browser || !this.snapshotMgr)
        ...
        const newHash = await this.snapshotMgr.store(program, identifier);
        ...
        const scopeAssets = extractScopeAssets(program);
        ...
        this.send('coordinator', 'SNAPSHOT:DIFF', {
            programUrl,
            platform,
            programName: program.program_name ?? programUrl,
            hasChanges,
            newHash,
...
function extractSlug(url) {
    try {
        ...
        if (parts.length >= 2 && (parts[0] === 'engagements' || parts[0] === 'programs'))
            return parts[1];
...
function extractScopeAssets(program) {
    const assets = [];
    ...
    if (Array.isArray(program.scope)) {
        for (const s of program.scope) {
            if (typeof s === 'string')
                ...
                assets.push(s.target);
            else if (s.identifier)
                assets.push(s.identifier);
        }
    }
    if (program.program_url)
        assets.push(program.program_url);
