...
export class ScannerAgent extends BaseAgent {
    ...
    async runFullScan(targets, strategies = {}) {
        if (this.scanning) {
        ...
        try {
            ...
            if (hasStrategies) {
                ...
                for (const [programUrl, groupTargets] of strategyGroups) {
                    ...
                    LOG.log(`Scanning ${groupTargets.length} targets with strategy for ${programUrl}: ` +
                    ...
                    try {
                        ...
                        if (result.findings.length > 0) {
                            ...
                            reportPaths.push(path.join(dir, `scan-${hash}.md`), path.join(dir, `scan-${hash}.json`));
                        }
                    }
                    catch (scanErr) {
     ...
     */
    buildToolConfigFromStrategy(strat) {
