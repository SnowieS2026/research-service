import { StackDetector } from '../stackdetector/StackDetector.js';
...
export class DiscoveryScanner {
     ...
     */
    async probe(url, timeoutMs = 15_000) {
     ...
     */
    async discoverEndpoints(pageUrl, timeoutMs = 20_000) {
        ...
        try {
            ...
            const jsUrls = await page.locator('script:not([src])').evaluateAll((els) => els.map((el) => el.textContent ?? ''));
            ...
            // External JS files
            const externalScripts = await page.locator('script[src]').evaluateAll((els) => els.map((el) => el.src));
     ...
     */
    async captureScreenshot(url, outputDir) {
        await this.ensureBrowser();
        if (!this.context)
            return null;
        const page = await this.context.newPage();
     ...
     */
    async scan(urls, outputDir = 'reports') {
        const results = [];
        const seen = new Set();
        // Deduplicate
