...
function buildStackCounts(stackInfos) {
    const counts = {};
...
export class ScannerOrchestrator {
    ...
    async scanTargets(targets) {
        ...
        const loginFormEndpoints = uniqueEndpoints.filter((e) => e.formFields.some((f) => f.type === 'password'));
        // 5. Execute scan tools in parallel (max 3 concurrent)
        const scanPromises = [];
        if (this.config.tools.dalfox && paramEndpoints.length > 0) {
            scanPromises.push(scanForXSS(paramEndpoints, discoveryResults[0]?.stackInfo ?? {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.sqlmap && paramEndpoints.length > 0) {
            scanPromises.push(scanForSQLi(paramEndpoints, discoveryResults[0]?.stackInfo ?? {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.ssrf && paramEndpoints.length > 0) {
            scanPromises.push(scanForSSRF(paramEndpoints, discoveryResults[0]?.stackInfo ?? {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.auth) {
            ...
            if (authTargets.length > 0) {
                scanPromises.push(scanForAuthIssues(authTargets, discoveryResults[0]?.stackInfo ?? {}, this.config)
                    .then((f) => {
                    allFindings.push(...f);
        ...
        if (this.config.tools.api) {
            scanPromises.push(scanAPI(uniqueEndpoints, discoveryResults[0]?.stackInfo ?? {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.nuclei && capped.length > 0) {
            ...
            scanPromises.push(runNuclei(capped, stackTechs, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.subfinder) {
            scanPromises.push(scanSubfinder(capped, {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.gau) {
            scanPromises.push(scanGau(capped, {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.httpx) {
            scanPromises.push(scanHttpx(capped, {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        if (this.config.tools.gitleaks) {
            scanPromises.push(scanGitleaks(capped, {}, this.config)
                .then((f) => {
                allFindings.push(...f);
        ...
        LOG.log(`ScannerOrchestrator: scan complete in ${duration}ms – ` +
    ...
    storeInDb(result) {
        ...
        try {
            // Insert scan run
            const scanId = this.db.insertScanRun(result.scanId, result.startedAt, result.duration, result.targetsScanned, result.summary.xss + result.summary.sql + result.summary.ssrf + result.summary.idor + result.summary.auth + result.summary.rce + result.summary.info, result.errors.length);
            // Insert findings
            for (const f of result.findings) {
                this.db.insertScanFinding(scanId, f.url, f.type, f.severity, f.cvss, f.tool, f.description, f.evidence);
            }
