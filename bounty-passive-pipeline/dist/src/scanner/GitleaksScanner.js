 ...
 */
function extractGitHubRepos(targets) {
    const repos = [];
    const seen = new Set();
    for (const target of targets) {
        ...
        if (gitMatch) {
            ...
            if (!seen.has(key)) {
                seen.add(key);
                repos.push({ org: gitMatch[1], repo: gitMatch[2] });
            }
            continue;
        }
        ...
        try {
            ...
            if (u.hostname === 'github.com' || u.hostname === 'www.github.com') {
                ...
                if (parts.length >= 2) {
                    ...
                    if (!seen.has(key)) {
                        seen.add(key);
                        repos.push({ org: parts[0], repo: parts[1] });
                    }
                }
            }
        }
 ...
 */
async function cloneRepo(repoUrl, tmpDir, timeoutMs = 60_000) {
 ...
 */
async function runGitleaks(repoPath, timeoutMs = 120_000) {
 ...
 */
export async function scanGitleaks(targets, _stack, config) {
    const findings = [];
    const hasGitleaks = await isToolAvailable('gitleaks');
    if (!hasGitleaks) {
