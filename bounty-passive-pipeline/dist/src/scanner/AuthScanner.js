 ...
 */
async function testJWT(endpoint, tokenValue) {
    const findings = [];
    try {
 ...
 */
async function testMissingAuth(endpoint) {
    const findings = [];
 ...
 */
export async function scanForAuthIssues(targets, stack, config) {
    const findings = [];
    ...
    for (const endpoint of targets) {
        ...
        const isLoginForm = endpoint.formFields.some(f => f.type === 'password' && (f.name.toLowerCase().includes('password') || f.name.toLowerCase().includes('pass')));
